...
local d_popup = require("deboost.keyboard_popup")
...
local globalkeys = gears.table.join(
    ...
    --brightness_widget:inc()
		awful.spawn.with_line_callback("bash -c 'brightnessctl s +40'", {
			exit = function()
	...
	awful.key({ modkey }, "[", function()
    --volume_widget:dec(5)
		awful.spawn.easy_async("bash -c 'amixer -D pulse sset Master 2%-'", function(stdout, stderr, exitreason, exitcode)
			--d_widget.volume_timer:emit_signal("timeout")
    end)
  end, { description = "dec volume", group = "audio" }),
	awful.key({ modkey }, "\\", function()
		awful.spawn.easy_async("bash -c 'amixer -D pulse set Master 1+ toggle'", function(stdout, stderr, exitreason, exitcode)
			--d_widget.volume_timer:emit_signal("timeout")
    end)
